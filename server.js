...
app.get("/api/weather/coords", async (req, res) => {
  ...
  try {
    ...
    console.error("❌ Weather (coords) proxy error:", err.message);
    res.status(500).json({ error: "Unable to fetch weather" });
  }
});
...
app.post("/api/itinerary", async (req, res) => {
  const { days, visitType, budget, interests = [] } = req.body;
  if (!days || !visitType || !budget) {
    return res.status(400).json({ error: "days, visitType, and budget are required" });
  }

  const prompt = `Create a detailed ${days}-day London itinerary for a ${visitType} trip. Interests include: ${interests.join(", ")}. Daily budget: ${budget}. Please include:
- Day-by-day breakdown with clear time slots (format as 9:00 AM - 11:00 AM)
- Specific attractions and activities through 8:00 PM
- After 8:00 PM or at the end of the day, add:
  * Transportation information (how to get around London)
  * Budget breakdown showing costs for the day
  * Tips and other useful information as bullet points
IMPORTANT: Please format with clear sections for each day. Put the schedule in a time-based structure up to 8:00 PM, then add the transportation, budget and tips information AFTER the timetable as separate bullet points.`;
  ...
  try {
    ...
    if (!text) {
      return res.status(502).json({ error: "No itinerary generated" });
    }
    res.json({ itinerary: text });
  } catch (err) {
    console.error("❌ Itinerary proxy error:", err.response?.data || err.message);
...
app.post("/api/itinerary", async (req, res) => {
  ...
  try {
    ...
    const text = response.data?.candidates?.[0]?.content?.parts?.[0]?.text;
    if (!text) return res.status(502).json({ error: "No itinerary generated" });
    res.json({ itinerary: text });
  } catch (err) {
    console.error("❌ Itinerary proxy error:", err.response?.data || err.message);
    res.status(500).json({ error: "Unable to generate itinerary" });
